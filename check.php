...
if (isset($_GET['payment_status'])) {
    if ($_GET['payment_status'] <= 0) {
       header('Location: https://example.com/noorder/click.php?message=Ошибка оплаты');
    }elseif ($_GET['payment_status'] == 1) {
        header('Location: https://example.com/noorder/click.php?message=Платёж находится в обработке');
